...
一款基于 PyQt5 的跨站脚本（XSS）Payload 生成器桌面工具，专为安全测试人员设计。提供 Payload 生成、筛选、上下文适配、变异、效果预览等功能，支持多种 WAF 绕过策略。
...
- Python 3.8+（推荐 3.11）
- Windows / macOS / Linux
- 推荐安装字体：[霞鹜文楷（LXGW WenKai）](https://github.com/lxgw/LxgwWenKai)（未安装时回退到 Microsoft YaHei）
...
```bash
git clone https://github.com/你的用户名/xss-payload-generator.git
...
├── core/
│   ├── __init__.py             # 导出 PayloadManager
│   └── payload_manager.py     # Payload 加载/筛选/管理核心逻辑
├── gui/
│   ├── __init__.py             # GUI 基础模块（公共导入、主题辅助）
...
├── xss_payload_generator.py   # XSS Payload 数据库 + 绕过/变异/验证逻辑
├── icons.py                    # SVG 图标定义
...
|---|---|---|
| PyQt5 | >= 5.15.0 | GUI 框架 |
| PyQt5-spi | >= 12.13.0 | PyQt5 插件支持（含 SVG 渲染） |

其余均为 Python 标准库（sys、os、json、re、html、base64、urllib.parse、tempfile、webbrowser、datetime）。
